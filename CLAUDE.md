...
2. **`build_files/build.sh`** — Executed during the container build (`RUN /ctx/build.sh`). Installs `freeipa-client`, `oddjob`, `oddjob-mkhomedir`, `powertop`; creates `/etc/ipa/` and `/etc/sssd/conf.d/` directory skeletons; declares `/var/lib/sss/` and `/var/log/sssd/` via `var-state.conf` (a `systemd-tmpfiles.d` drop-in, so no `/var` content is baked into the image layer — a real first boot creates them); ships `trayscale-flatpak-install.service` (installs the Trayscale Flatpak app on first boot) and `powertop-autotune.service` (runs `powertop --auto-tune` every boot); creates the `linuxbrew` user and `brew` group via `systemd-sysusers` with pinned UID/GID 950/951 (`homebrew-sysusers.conf`) and ships `homebrew-install.service` (installs Homebrew to `/home/linuxbrew/.linuxbrew` on first boot, via `homebrew-install.sh`); enables `sssd`, `oddjobd`, `podman.socket`, `trayscale-flatpak-install.service`, `powertop-autotune.service`, and `homebrew-install.service`. Runs with `set -ouex pipefail`.
